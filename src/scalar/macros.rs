macro_rules! define_quantity {
    ($name:ident) => {
        ...
        impl $name {
            ...
            where
                U: $crate::scalar::Unit<Quantity = Self>,
            {
...
macro_rules! define_unit {
    ($(#[$meta:meta])* $name:ident : $quantity:ty, scale = $scale:expr) => {
        ...
        impl $crate::scalar::Unit for $name {
    ...
    ($(#[$meta:meta])* $name:ident : $quantity:ty, scale = $scale:expr, prefixable) => {
        $crate::scalar::macros::define_unit!(
            $(#[$meta])*
            $name : $quantity,
            scale = $scale
        );

        impl $crate::scalar::PrefixableUnit for $name {}
    };
}
...
macro_rules! define_prefix {
    ($name:ident, factor = $factor:expr) => {
        ...
        impl $crate::scalar::Prefix for $name {
